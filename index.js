...
const createManager = () => {
    const depatmentQuery = `SELECT roles.id AS role_id, roles.role_title, departments.id AS dept_id, departments.dept_title FROM roles INNER JOIN departments ON roles.id = departments.id`
...
const createSubordinate = () => {
        const roleQuery = `SELECT role_title FROM roles WHERE role_title != ('Manager')`
        db.query(roleQuery, (err, res) => {
            const roleChoices = res.map(({ id, role_title }) => ({
                name: role_title,
                value: id
